...
sigmaApp.controller('EmailListCtrl', function($scope, $http, Emails) {
  ...
  $scope.showingClass = function(showingVar) {
	...
	$scope.archive = function(email) {
		var archived = arguments.length > 1 ? arguments[1] : true;
		// alert(archived);
		if (window.location.search != "?home") {
			$http({
				method: 'POST',
  ...
  $scope.viewing = function(emailId) {
		...
		// setTimeout('$(".viewing-padding").height($(".viewing-bar").height())', 150);

		if (!$scope.viewingEmail.html) {
			$('.viewing-message').html("<div class='no-html-email-view'>" + $scope.viewingEmail.message + "</div>");
		  }
		  else {
			$('.viewing-message').html("<iframe class='email-frame' frameBorder='0' src='/get_email?id=" + $scope.viewingEmail.id + "' ></iframe>");
	...
	$scope.tbl_values = function() {
		...
		if(!$scope.initializedTBL) {
			...
			$scope.cc_tbl = new $.TextboxList("#compose-cc", {unique: true, placeholder : "CC" /*, plugins: {autocomplete: {}} */});
			$scope.bcc_tbl = new $.TextboxList("#compose-bcc", {unique: true, placeholder : "BCC" /*, plugins: {autocomplete: {}} */});
...
*/
	...
	$scope.compose = function() {
		...
		if(arguments.length > 0) {
			...
			$scope.compose_tbl.add(1, arguments[0].fromEmail, arguments[0].fromEmail);
			$scope.emails.contacts[1][4] = arguments[0].fromEmail;
			$("#compose-body").val("\n\n---------------------------------\nOn "
